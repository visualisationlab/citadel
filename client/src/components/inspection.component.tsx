...
import { Container, Form, Button, Row, Col, Stack, Table, ListGroup } from 'react-bootstrap'
...
function ClusterTab(
    ...
    if (attributeList.every((attribute) => {
        ...
        const vals = attributeList.map((e) => {return parseFloat(e)})
                                ...
                                return (
                                    <Stack key={key}>
                                        ...
                                        <Form.Control id={key}
                                            ...
                                            type="text"
                                            value={attributes[key] === (undefined) ? '' : attributes[key]?.toString()}

                                            placeholder={attributes[key] === (undefined) ? '' : attributes[key]?.toString()}/>
                                    </Stack>
                                )
                            })
                        }
                        <hr></hr>
                        {
                            showHidden ? (
                                Object.keys(attributes).filter(
                                    ...
                                    >
                                        {
                                            key.startsWith('_') ? <i>{key.slice(1)}</i> : key
                                        }
                                    </h6>
                                    <Form.Control id={key}
                                        onChange={
        ...
        <div
            ...
            onMouseDown={() => {
                ...
                window.onmousemove = ((e) => {
                    const delta = - props.barWidth / 2

                    const x = props.position === 'right' ? window.innerWidth - e.clientX : e.clientX
                    // if (dragging) {
